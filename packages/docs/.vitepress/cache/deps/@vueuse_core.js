...
// ../../node_modules/.pnpm/registry.npmmirror.com+@vueuse+shared@9.13.0/node_modules/@vueuse/shared/index.mjs
...
// ../../node_modules/.pnpm/registry.npmmirror.com+@vueuse+core@9.13.0/node_modules/@vueuse/core/index.mjs
...
function onLongPress(target, handler, options) {
    ...
    function onDown(ev) {
        var _a22, _b2, _c, _d
        if (((_a22 = options == null ? void 0 : options.modifiers) == null ? void 0 : _a22.self) && ev.target !== elementRef.value) return
        clear()
...
function useDevicesList(options = {}) {
    const { navigator = defaultNavigator, requestPermissions = false, constraints = { audio: true, video: true }, onUpdated: onUpdated2 } = options
    const devices = ref([])
...
function useFetch(url, ...args) {
    ...
    const execute = async (throwOnFailed = false) => {
        ...
        if (config.payload) {
            ...
            if (config.payloadType) headers['Content-Type'] = (_a22 = payloadMapping[config.payloadType]) != null ? _a22 : config.payloadType
...
function useMouse(options = {}) {
    const { type = 'page', touch = true, resetOnTouchEnds = false, initialValue = { x: 0, y: 0 }, window: window2 = defaultWindow, eventFilter } = options
...
var __objRest$12 = (source, exclude) => {
    var target = {}
    for (var prop in source) if (__hasOwnProp$82.call(source, prop) && exclude.indexOf(prop) < 0) target[prop] = source[prop]
...
function useOffsetPagination(options) {
    const { total = Infinity, pageSize = 10, page = 1, onPageChange = noop, onPageSizeChange = noop, onPageCountChange = noop } = options
    ...
    async function read(event) {
        ...
        try {
            ...
            if (rawValue == null) {
                ...
                else if (type === 'object' && !Array.isArray(value)) data.value = __spreadValues$42(__spreadValues$42({}, rawInit), value)
...
var __objRest2 = (source, exclude) => {
    var target = {}
    for (var prop in source) if (__hasOwnProp$22.call(source, prop) && exclude.indexOf(prop) < 0) target[prop] = source[prop]
...
function useUrlSearchParams(mode = 'history', options = {}) {
    const { initialValue = {}, removeNullishValues = true, removeFalsyValues = false, write: enableWrite = true, window: window2 = defaultWindow } = options
