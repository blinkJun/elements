import { isActive, normalizeLink, isExternal } from 'vitepress/dist/client/theme-default/support/utils'
import { inBrowser } from 'vitepress'

import type { Route } from 'vitepress'

export * from 'vitepress/dist/client/theme-default/index'
...
export const isActiveLink = (route: Route, pathPattern: string, match?: boolean) => {
    ...
    return regex.test(normalizeLink(`/${route.data.relativePath}`))
}

export function createGitHubUrl(docsRepo: string, docsDir: string, docsBranch: string, path: string, folder = 'examples/', ext = '.vue') {
    const base = isExternal(docsRepo) ? docsRepo : `https://github.com/${docsRepo}`
    return `${base}/edit/${docsBranch}/${docsDir ? `${docsDir}/` : ''}${folder || ''}${path}${ext || ''}`
}
